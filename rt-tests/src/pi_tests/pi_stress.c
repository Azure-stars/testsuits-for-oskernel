...
void *low_priority(void *arg)
{
	...
	if (status && status != PTHREAD_BARRIER_SERIAL_THREAD)
	{
		error("low_priority[%d]: pthread_barrier_wait(all_threads_ready): %x",
			  p->id, status);
		return NULL;
	}
	...
	if (status && status != PTHREAD_BARRIER_SERIAL_THREAD)
	{
		error("low_priority[%d]: pthread_barrier_wait(all_threads_done): %x",
			  p->id, status);
		return NULL;
	}
...
void *med_priority(void *arg)
{
	...
	if (status && status != PTHREAD_BARRIER_SERIAL_THREAD)
	{
		error("med_priority[%d]: pthread_barrier_wait(all_threads_ready): %x",
			  p->id, status);
		return NULL;
	}
	...
	for (;;)
	{
		...
		if (status && status != PTHREAD_BARRIER_SERIAL_THREAD)
		{
			error("med_priority[%d]: pthread_barrier_wait(start): %x",
				  p->id, status);
			return NULL;
		}
		...
		if (status && status != PTHREAD_BARRIER_SERIAL_THREAD)
		{
			error("med_priority[%d]: pthread_barrier_wait(finished): %x",
				  p->id, status);
			return NULL;
		}
	}
...
void *high_priority(void *arg)
{
	...
	for (;;)
	{
		...
		if (status && status != PTHREAD_BARRIER_SERIAL_THREAD)
		{
			error("high_priority[%d]: pthread_barrier_wait(start): %x",
				  p->id, status);
			return NULL;
		}
		...
		if (status && status != PTHREAD_BARRIER_SERIAL_THREAD)
		{
			error("high_priority[%d]: pthread_barrier_wait(running): %x",
				  p->id, status);
			return NULL;
		}
		...
		if (status && status != PTHREAD_BARRIER_SERIAL_THREAD)
		{
			error("high_priority[%d]: pthread_barrier_wait(finish): %x",
				  status);
			return NULL;
		}
